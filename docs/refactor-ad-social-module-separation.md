# Plan: app.js 저결합 모듈 분리 (광고 → 소셜 순서, IIFE 패턴 유지)
...
- Module Bridge (app.js line 15215)에서 `window.AppState`, `window.saveUserData` 등 공유 상태 노출

---

# Phase 1: 광고 모듈 (`modules/ad-manager.js`)
...
- `/home/user/levelup-app/modules/ad-manager.js` (신규)
- `/home/user/levelup-app/www/modules/ad-manager.js` (www 동기화)

---

# Phase 2: 소셜 모듈 (`modules/social.js`)
...
- `/home/user/levelup-app/modules/social.js` (신규)
- `/home/user/levelup-app/www/modules/social.js` (www 동기화)

---

# Phase 3: 동적 로드 등록 & www 동기화
...
| 파일 | 작업 | Phase |
|------|------|-------|
| `modules/ad-manager.js` | **신규** — 광고 IIFE 모듈 (약 650줄) | 1 |
| `app.js` | 광고 코드 제거 + 호출부 수정 + Module Bridge 확장 | 1 |
| `modules/social.js` | **신규** — 소셜 IIFE 모듈 (약 230줄) | 2 |
| `app.js` | 소셜 코드 제거 + 호출부 수정 + Module Bridge 추가 확장 | 2 |
| `app.js` | 동적 import 등록 | 3 |
...
1. **구문 검증**: `node --check modules/ad-manager.js`, `node --check modules/social.js`
2. **참조 무결성**: app.js에서 제거한 함수명이 직접 호출되지 않고 `window.*` 경유로만 참조되는지 grep 확인
3. **로드 순서**: Module Bridge → ad-manager.js → social.js → exercise-calc.js 확인
4. **방어적 호출**: 모듈 미로드 시 `if(window.AdManager)` / `if(window.SocialModule)` 가드 확인
5. **window 노출**: HTML onclick 핸들러(`fetchSocialData`, `toggleFriend`, `goToSocialTab`, `claimBonusExp`)가 window에 올바르게 부착되었는지 확인
