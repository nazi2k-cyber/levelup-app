...
**하이브리드 이미지 스크리닝 (NSFWJS 1차 무료 → Azure Content Safety 2차 정밀)** 을 통해
부적절한 콘텐츠를 자동으로 감지하고 조치합니다.
...
| 이미지 검열 불가 | NSFWJS + Azure 하이브리드로 성인/폭력/혐오/자해 감지 |

### 이미지 스크리닝: 하이브리드 방식 채택

```
이미지 업로드
    │
    ▼
[1차] NSFWJS 로컬 추론 (무료, 무제한)
    │
    ├── Porn/Hentai > 80% → 즉시 HIGH 플래그 (Azure 호출 안함)
    ├── Neutral/Drawing > 90% → 안전 통과 (Azure 호출 안함)
    │
    └── 애매한 결과 (Sexy > 30% 등)
         │
         ▼
[2차] Azure Content Safety (F0: 5,000건/월 무료)
         │
         └── Sexual, Violence, Hate, SelfHarm 정밀 분석
```

| 단계 | 솔루션 | 비용 | 감지 범위 | 처리 비율 |
|------|--------|------|---------|---------|
| **1차** | NSFWJS (로컬) | $0 무제한 | 성적 콘텐츠 (Porn, Sexy, Hentai) | ~88% 처리 |
| **2차** | Azure Content Safety | F0: 5,000건/월 무료 | Sexual, Violence, Hate, SelfHarm | ~12% (애매한 건만) |

> **Azure API 호출 85%+ 절감** — 전체 이미지의 ~12%만 Azure로 전달
...
```javascript
{
  ...
  // 이미지 스크리닝 결과 (하이브리드: NSFWJS 또는 Azure)
  imageFlags: {
    adult: "VERY_UNLIKELY",         // Sexual/Porn 감지
    violence: "UNLIKELY",           // Violence 감지 (Azure만)
    racy: "POSSIBLE",               // Sexy/선정적 감지
    hate: "VERY_UNLIKELY",          // Hate 감지 (Azure만)
    selfHarm: "VERY_UNLIKELY",      // SelfHarm 감지 (Azure만)
    _source: "nsfwjs",              // "nsfwjs" 또는 "azure" (판정 소스)
...
screenImage(photoUrl, settings)
  → 하이브리드 이미지 스크리닝 실행
  → 1차: NSFWJS 로컬 추론 (Porn/Hentai >80% → 즉시 플래그, Neutral/Drawing >90% → 통과)
  → 2차: 애매한 결과만 Azure Content Safety API로 정밀 검사
  → adult, violence, racy, hate, selfHarm 결과 반환

screenImageLocal(photoUrl)
  → NSFWJS MobileNetV2 모델로 로컬 추론 (무료)
  → Porn, Sexy, Hentai, Neutral, Drawing 확률 반환

screenImageAzure(photoUrl)
  → Azure Content Safety API 호출 (이미지 URL → base64 → 분석)
  → Sexual, Violence, Hate, SelfHarm 분석 결과 반환
...
- **상세 패널**:
  ...
  - 포스트 이미지
  - 텍스트 플래그 태그 (키워드 + 카테고리)
  - 이미지 플래그 태그 (NSFWJS 또는 Azure: 성인, 폭력, 선정, 혐오, 자해 + 소스 표시)
...
- **일반 설정**
  - 텍스트 스크리닝 활성화 토글
  - 이미지 스크리닝 활성화 토글 (NSFWJS 1차 무료)
  - Azure 2차 정밀검사 토글 (F0: 5,000건/월 무료, 애매한 결과만 호출)
  - 플래그 알림 토글
...
| `functions/package.json` | 수정 | `nsfwjs`, `@tensorflow/tfjs-node`, `@azure-rest/ai-content-safety`, `@azure/core-auth` |
...
2. (이미지 스크리닝 사용 시) Cloud Functions 메모리 설정:
   - NSFWJS 모델 로딩에 **최소 512MB, 권장 1GB** 메모리 필요
   - `firebase.json`에서 `functions.memory: "1GiB"` 설정 또는 GCP Console에서 조정
3. (Azure 2차 정밀검사 사용 시) Azure Content Safety 리소스 생성:
...
4. 금칙어 목록 확인/수정
5. 텍스트 스크리닝 활성화 확인
6. 이미지 스크리닝 활성화 (NSFWJS 1차 무료)
7. (선택) Azure 2차 정밀검사 활성화 (F0: 5,000건/월 무료)
7. **설정 저장** 클릭
...
| 단계 | 솔루션 | 비용 |
|------|--------|------|
| 1차 필터 | NSFWJS (로컬) | **$0 (무제한)** |
| 2차 정밀 | Azure Content Safety F0 | **$0 (5,000건/월 무료)** |
| 2차 정밀 | Azure Content Safety S0 | $1.00 / 1,000건 |

### 하이브리드 시나리오별 예상 비용

NSFWJS가 ~88%를 로컬 처리하므로 Azure는 ~12%만 호출됩니다.

| 월간 이미지 수 | Azure 호출 (~12%) | 하이브리드 비용 | (참고) Azure 단독 | (참고) Google Vision |
|-------------|-----------------|--------------|-----------------|-------------------|
| 1,000건 | ~120건 | **$0** (F0) | $0 (F0) | $0 |
| 5,000건 | ~600건 | **$0** (F0) | $0 (F0) | $6.00 |
| 10,000건 | ~1,200건 | **$0** (F0) | $10.00 (S0) | $13.50 |
| 30,000건 | ~3,600건 | **$0** (F0) | $30.00 (S0) | $43.50 |
| 50,000건 | ~6,000건 | **$1.00** (S0) | $50.00 (S0) | $73.50 |

> **월 ~40,000건까지 완전 무료** (Azure F0 5,000건 한도 ÷ 12% = ~41,667건)
>
...
| ~~NSFWJS 하이브리드~~ | ~~1차 로컬 필터 → 2차 Azure (비용 85% 절감)~~ | **구현 완료** |
