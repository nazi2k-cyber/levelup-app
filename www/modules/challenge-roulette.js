...
(function() {
    ...
    function getWeeklyChallenges() {
        const weekStart = window.getWeekStartDate();
    ...
    window.claimChallenge = function(idx) {
        ...
        const statKeys = window.statKeys;
    ...
    function startRouletteTimer() {
        ...
        function tick() {
            ...
            const lang = AppState.currentLang;
            timerEl.textContent = `${i18n[lang].roulette_next_spin} ${window.formatCountdown(ms)}`;
    ...
    // 광고 보상형 스핀 보너스 (applyRewardedInterstitialBonus에서 호출)
    function applySpinBonus() {
        const lang = AppState.currentLang;
        const statKeys = window.statKeys;
