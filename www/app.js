...
function switchTab(tabId, el) {
    ...
    if(tabId === 'status') {
        ...
        drawRadarChart(); updatePointUI(); window.renderQuote?.(); window.renderDDayList?.(); window.renderDDayCaption?.(); renderLifeStatus(); if (window.AdManager) window.AdManager.renderBonusExp(); if (window.updateLibraryCardCount) window.updateLibraryCardCount(); if (window.updateMovieCardCount) window.updateMovieCardCount();
    } else {
        mainEl.style.overflowY = 'auto';
    }
