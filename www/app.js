...
import { doc, setDoc, getDoc, deleteDoc, collection, getDocs, query, where, orderBy, limit, updateDoc, arrayUnion, arrayRemove, enableNetwork, disableNetwork } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
function bindEvents() {
    ...
    document.getElementById('btn-bg-theme').addEventListener('click', openBackgroundThemeModal);
    document.getElementById('btn-bg-theme-close').addEventListener('click', closeBackgroundThemeModal);
    document.getElementById('bg-theme-modal').addEventListener('click', (e) => { if (e.target.id === 'bg-theme-modal') closeBackgroundThemeModal(); });
...
window.completeDungeon = () => {
    ...
    // ★ 보상형 전면 광고 — 레이드 수행 종료 후 팝업 노출 (일 1회)
    if (window.AdManager
        ...
        setTimeout(() => {
            const watchAd = confirm(i18n[lang].ri_dungeon_prompt || '레이드 수행이 종료되었습니다. 광고를 시청하면 추가 보상을 받을 수 있습니다. 시청하시겠습니까?');
            if (watchAd) {
...
function openDiyQuestInfoModal() {
    const lang = AppState.currentLang;
    document.getElementById('info-modal-title').innerText = i18n[lang].diy_guide_title || "사이드 퀘스트 가이드";
    const body = document.getElementById('info-modal-body');
    ...
    const guideData = {
        ko: {
            sections: [
                ...
                { icon: '🔢', title: '최대 6개 생성 가능 (프리미엄 최대 36개)', desc: '사이드 퀘스트는 최대 6개까지 만들 수 있습니다. 프리미엄 회원은 최대 36개까지 생성 가능합니다. 불필요한 퀘스트는 삭제 후 새로 만들어 주세요.' },
                ...
                { icon: '🗓️', title: '플래너 연동', desc: '생성한 사이드 퀘스트는 플래너의 우선순위 태스크 영역에도 자동으로 표시됩니다. 퀘스트 탭과 플래너 탭 어디서든 완료 처리할 수 있으며, 완료 상태는 양쪽에 실시간으로 반영됩니다.' }
            ]
        },
        en: {
            sections: [
                ...
                { icon: '🔢', title: 'Up to 6 Quests (Premium: 36)', desc: 'You can create up to 6 Side quests (36 for Premium members). Delete unused quests to make room for new ones.' },
                ...
                { icon: '🗓️', title: 'Planner Integration', desc: 'Your Side quests automatically appear in the Planner\'s priority tasks section. You can mark them complete from either the Quest tab or the Planner tab — completion status syncs in real time between both.' }
            ]
        },
        ja: {
            sections: [
                { icon: '📝', title: '自分だけのクエスト作成', desc: '自分の目標をクエストとして登録できます。タイトル(最大20文字)、説明(最大40文字)、ステータスを選択してください。' },
                { icon: '🔢', title: '最大6個まで作成可能（プレミアム: 最大36個）', desc: 'サイドクエストは最大6個まで（プレミアム会員は最大36個）作成できます。不要なクエストは削除してから新しく作成してください。' },
                ...
                { icon: '🗓️', title: 'プランナー連動', desc: '作成したサイドクエストはプランナーの優先タスクエリアにも自動表示されます。クエストタブとプランナータブのどちらからでも完了処理でき、完了状態はリアルタイムで両方に反映されます。' }
            ]
        }
    };
...
function _updateChipFillVisuals() {
    const bank = document.getElementById('planner-task-chips-bank');
    if (!bank) return;
    bank.querySelectorAll('.task-chip').forEach(chip => {
        chip.classList.toggle('chip-active', chip.dataset.task === timeboxFillTask);
        chip.classList.toggle('chip-inactive', !!timeboxFillTask && chip.dataset.task !== timeboxFillTask);
    });
}

function enterTimeboxFillMode(taskText) {
    timeboxFillTask = taskText;
    _updateChipFillVisuals();
    const grid = document.getElementById('planner-timebox-grid');
    if (grid) grid.classList.add('fill-mode-active');
}

function exitTimeboxFillMode() {
    timeboxFillTask = null;
    _updateChipFillVisuals();
    const grid = document.getElementById('planner-timebox-grid');
    if (grid) grid.classList.remove('fill-mode-active');
}

function initTimeboxChipClickFill() {
    const bank = document.getElementById('planner-task-chips-bank');
    if (!bank || bank.dataset.chipClickInit) return;
    bank.dataset.chipClickInit = '1';
    bank.addEventListener('click', (e) => {
        const chip = e.target.closest('.task-chip');
        if (!chip) return;
        const taskText = chip.dataset.task;
        if (timeboxFillTask === taskText) exitTimeboxFillMode();
        else enterTimeboxFillMode(taskText);
    });
}

function initTimeboxFillMode() {
    const grid = document.getElementById('planner-timebox-grid');
    if (!grid || grid.dataset.fillInit) return;
    grid.dataset.fillInit = '1';
    grid.addEventListener('click', (e) => {
        if (!timeboxFillTask) return;
        const slot = e.target.closest('.timebox-slot');
        if (!slot || slot.classList.contains('slot-disabled')) return;
        setSlotValue(slot, timeboxFillTask);
    });
}
...
// 슬롯 HTML 생성 헬퍼 (이전의 initTimeboxChipDrag는 fill mode로 대체됨)
...
function initBackgroundTheme() {
    renderBackgroundThemeTiles();
    const saved = localStorage.getItem('backgroundTheme') || 'default';
    applyBackgroundTheme(saved);
}
...
function applyBackgroundTheme(themeId) {
    ...
    if (selected.size) document.body.style.backgroundSize = selected.size;
